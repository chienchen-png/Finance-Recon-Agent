...
- 用户请求创建新工作区、搜索/安装扩展、打开浏览器、页面交互或执行 VS Code 命令时，先确认这些能力是否能通过现有工具链实现。
- 需要确认 `.agent.md` 是否显式授权 `read/edit/search/execute/web/agent/todo` 以及 `vscode_askQuestions`、`memory`、`extensions`、`installExtension`、`newWorkspace`、`resolveMemory`、`runCommand`、`toolSearch` 和 Python 扩展工具 `configurePythonEnvironment/getPythonEnvironmentInfo/getPythonExecutableCommand/installPythonPackage`。
